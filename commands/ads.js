...
module.exports = {
    data: new SlashCommandBuilder().setName('ads').setDescription('Request Streamer/Youtuber role')
      .addStringOption(option => option.setName('url').setDescription('Twitch/Youtube URL').setRequired(true)),
    async execute(interaction) {
      if (!requiredKeys.every(key => nconf.get(key))) return
      const url = interaction.options.getString('url')
      const botChannel = await interaction.guild.channels.fetch(nconf.get('CHANNEL_BOT'))
      const urlHash = crypto.createHash('md5').update(url).digest('hex')
      urlStore.set(urlHash, url)
      ...
      const actionRow = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId(`grant_youtuber_role:${interaction.user.id}:${urlHash}`).setLabel('Give Youtuber Role').setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId(`grant_streamer_role:${interaction.user.id}:${urlHash}`).setLabel('Give Streamer Role').setStyle(ButtonStyle.Success),
        new ButtonBuilder().setCustomId(`deny_role_request:${interaction.user.id}:${urlHash}`).setLabel('Deny Role Request').setStyle(ButtonStyle.Danger)
      )
    ...
    async handleRoleRequestButtons(interaction) {
      ...
      const [action, userId, urlHash] = interaction.customId.split(':')
      const url = urlStore.get(urlHash)
      urlStore.delete(urlHash)
