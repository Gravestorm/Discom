...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (user && channel) await interaction.channel.messages.fetch({ limit: 100, cache: false }).then(msgs => msgs.filter(m => m.author.id === user.id).first(num).reverse().forEach(m => { m.delete(); channel.send({ embeds: [new MessageEmbed().setAuthor({ name: m.author.username, iconURL: m.author.displayAvatarURL() }).setDescription(m.content).setImage(m.attachments.first() ? m.attachments.first().proxyURL : '').setFooter({ text: `#${m.channel.name}` }).setTimestamp(m.createdTimestamp).setColor(random())] }) }))
