...
module.exports = {
  ...
  async execute(interaction) {
    ...
    await fetch({ url: `https://www.krosmoz.com/en/almanax/${date}?game=dofus`, encoding: 'utf8', transform: (body) => { return cheerio.load(body) } }).then(async ($) => {
      ...
      await interaction.reply({ content: dc > df && dc < dt ? ' ' : 'Correct date format: yyyy-mm-dd (e.g. 2022-12-31)', embeds: [new MessageEmbed().setTitle(`**${date} | ${d.N} | ${d.R}**`).setDescription(`**Objet:** ${d.IFR}\n${d.BFR}\n\n\n**Item:** ${d.IEN}\n${d.BEN}`).setThumbnail(d.I).setColor(random())], ephemeral: true })
    }).catch(err => { throw err })
  }
}